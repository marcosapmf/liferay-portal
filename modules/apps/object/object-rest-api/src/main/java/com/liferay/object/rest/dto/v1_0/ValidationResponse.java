...
public class ValidationResponse implements Serializable {
	...
	public ValidationError[] getValidationErrors() {
		if (_validationErrorsSupplier != null) {
			validationErrors = _validationErrorsSupplier.get();

			_validationErrorsSupplier = null;
		}
	...
	public void setValidationErrors(
		UnsafeSupplier<ValidationError[], Exception>
			validationErrorsUnsafeSupplier) {

		_validationErrorsSupplier = () -> {
			try {
				return validationErrorsUnsafeSupplier.get();
			}
	...
	protected ValidationError[] validationErrors;

	@JsonIgnore
	private Supplier<ValidationError[]> _validationErrorsSupplier;
	...
	public String toString() {
		...
		ValidationError[] validationErrors = getValidationErrors();
		...
		if (validationErrors != null) {
			...
			for (int i = 0; i < validationErrors.length; i++) {
				sb.append(String.valueOf(validationErrors[i]));
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		...
		defaultValue = "com.liferay.object.rest.dto.v1_0.ValidationResponse",
		name = "x-class-name"
	)
