			...
			>
				...
				%>
					...
					>
						...
						<%
						Locale defaultLocale = ddmFormViewFormInstanceRecordsDisplayContext.getDefaultLocale(formInstanceRecord);
						%>

						<div class="search-container-column-language">
							<svg class="lexicon-icon lexicon-icon-<%= LocaleUtil.toW3cLanguageId(defaultLocale).toLowerCase() %> reference-mark user-icon-xl">
								<use xlink:href="<%= themeDisplay.getPathThemeImages() %>/clay/icons.svg#<%= LocaleUtil.toW3cLanguageId(defaultLocale).toLowerCase() %>" />
							</svg>
						</div>
					</liferay-ui:search-container-column-text>
