...
public class OSBFaroSitePortalInstanceLifecycleListener
			...
			_log.warn(
				"Site initializer bundle not found: " +
					_SITE_INITIALIZER_BSN +
						". Deploy osb-faro-site-initializer first.");
	...
	private void _initializeSiteForAllCompanies(Bundle bundle) {
		try {
			...
			_log.error("Failed to initialize OSB Faro site", exception);
		}
	}
		...
		if (group != null) {
			...
			if (layout != null) {
				if (_log.isInfoEnabled()) {
					_log.info(
						"OSB Faro site already fully initialized for " +
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					"OSB Faro site group exists but has no home page " +
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					"Creating OSB Faro site for company " + companyId);
			}
		}
	...
	private static final String _SITE_ERC = "OSB_FARO";

	private static final String _SITE_INITIALIZER_BSN =
		"osbfarositeinitializer";
